...
def _render_review_body(
    ...
    lines: list[str] = [
        f"## {bucket_emoji} AI Code Review — Risk Score: {risk_score}/100",
        "",
        "### Summary",
        summary,
        "",
    ...
    if findings:
        ...
        for f in findings:
            path = f.get("path", "—")
            line_no = f.get("line", "")
            loc = f"`{path}:L{line_no}`" if line_no else f"`{path}`"
            lines.append(
        ...
        for f in findings:
            if suggestion := f.get("suggestion"):
                ...
                cat = f.get("category", "")
                lines.append(f"<details><summary><b>{sev}</b> — {cat}</summary>")
                lines.append("")
                if path := f.get("path"):
                    ln = f.get("line", "")
                    lines.append(f"> `{path}`{'`:L' + str(ln) + '`' if ln else ''}")
                lines.append("")
    ...
    lines.append(f"### Verdict: {verdict_map.get(risk_bucket, 'ℹ️ COMMENT')}")
    lines.append("")
    lines.append("---")
    lines.append("*Generated by [MergeGuard](https://github.com/mergeguard) — "
                 "AI PR Review powered by AWS Strands SDK + Amazon Bedrock*")
